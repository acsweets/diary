...
class CustomPageRoute extends PageRouteBuilder {
  ...
  CustomPageRoute({required this.child, required this.lottieAnimationPath})
      : super(
            pageBuilder: (context, animation, secondaryAnimation) => child,
            transitionDuration: const Duration(milliseconds: 500),
            transitionsBuilder: (BuildContext context, animation, secondaryAnimation, Widget child) => FadeTransition(
                 //从上一个页面切到下一个页面的透明度变化
                  opacity: Tween(begin: 0.1, end: 1.0).animate(CurvedAnimation(
                    parent: animation,
                    curve: Curves.fastOutSlowIn,
                  )),
                  child: RoutePage( lottieAnimationPath:lottieAnimationPath, child: child ),
                ));
}
class RoutePage extends StatefulWidget {
  final Widget child;
  final String lottieAnimationPath;
  const RoutePage({Key? key, required this.child, required this.lottieAnimationPath}) : super(key: key);
