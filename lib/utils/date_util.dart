...
class DateUtil {
  ...
  static bool isWeek(int milliseconds, {bool isUtc = false}) {
    ...
    DateTime o_old = DateTime.fromMillisecondsSinceEpoch(milliseconds, isUtc: isUtc);
    DateTime n_now = isUtc ? DateTime.now().toUtc() : DateTime.now().toLocal();
    DateTime old = n_now.millisecondsSinceEpoch > o_old.millisecondsSinceEpoch ? o_old : n_now;
    DateTime now = n_now.millisecondsSinceEpoch > o_old.millisecondsSinceEpoch ? n_now : o_old;
  ...
  static bool isEqualDayByMs(int milliseconds1, int milliseconds2) {
    if (milliseconds1 <= 0 || milliseconds2 <= 0) return false;
    return isEqualDay(
